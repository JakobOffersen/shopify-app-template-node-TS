...
vi.mock(`${process.cwd()}/helpers/ensure-billing.js`, async () => {
  const actualModule = await vi.importActual(`${process.cwd()}/helpers/ensure-billing.js`)
...
describe('shopify-app-template-node server', async () => {
  ...
  describe('content-security-policy', () => {
    test('sets Content Security Policy for embedded apps', async () => {
      ...
      expect(response.headers['content-security-policy']).toEqual(`frame-ancestors https://test-shop.myshopify.test https://admin.shopify.com;`)
    })
  ...
  test('goes to top level auth in oauth flow when there is no cookie', async () => {
    const response = await request(app).get('/api/auth?shop=test-shop.myshopify.test').set('Accept', 'text/html')
  ...
  test('renders toplevel auth page', async () => {
    const response = await request(app).get('/api/auth/toplevel?shop=test-shop').set('Accept', 'text/html')
  ...
  describe('handles the callback correctly', () => {
    test('redirects to / with the shop and host if nothing goes wrong', async () => {
      ...
      const response = await request(app).get('/api/auth/callback?host=test-shop-host&shop=test-shop')
    ...
    test('returns 400 if oauth is invalid', async () => {
      ...
      const response = await request(app).get('/api/auth/callback?host=test-shop-host')
  ...
  describe('webhook processing', () => {
    ...
    test('processes webhooks', async () => {
      ...
      const response = await request(app)
        .post('/api/webhooks')
        .set('X-Shopify-Hmac-Sha256', createHmac('sha256', Shopify.Context.API_SECRET_KEY).update('{}', 'utf8').digest('base64'))
  ...
  describe('with billing enabled', async () => {
    ...
    test('redirects to a payment confirmation URL', async () => {
      ...
      billingMock.mockImplementation(() => Promise.resolve([false, 'payment-confirmation-url']))

      const response = await request(appWithBilling).get('/api/auth/callback?host=test-shop-host&shop=test-shop')
